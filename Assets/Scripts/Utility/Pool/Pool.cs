...
namespace KAP.Pool
{
    public static class Pool
    {
        ...
        /// <summary>This allows you to spawn a prefab with GameObject.</summary>
        public static GameObject Spawn(GameObject prefab, Vector3 position, Transform parent = null) => Spawn(prefab, position, Quaternion.identity, parent);
        ...
        #region Spawn T
        public static T Spawn<T>(T template, Vector3 position, Quaternion rotation, Transform parent = null)
            where T : Component
        {
            if (template == null) { Debug.LogError("Attempting to spawn a null prefab."); return null; }
            var clone = Spawn(template.gameObject, position, rotation, parent); 
